...
struct GuardrailsTypesTests {
    ...
    @Test func testDecodeGuardrail() throws {
        let json = Data("""
        {
            ...
            "ignored_models": ["meta-llama/llama-3"],
            "enforce_zdr": true,
        ...
        #expect(guardrail.ignoredModels == ["meta-llama/llama-3"])
        #expect(guardrail.enforceZdr == true)
        #expect(guardrail.enforceZdrOpenai == true)
        #expect(guardrail.workspaceId == "ws_123")
    ...
    @Test func testDecodeGuardrailMinimalFields() throws {
        ...
        #expect(guardrail.workspaceId == nil)
    }

    @Test func testDecodeContentFilterEntries() throws {
        let json = Data("""
        {
            "id": "gr_789",
            "name": "Filtered",
            "content_filter_builtins": [
                {"action": "block", "slug": "email"}
            ],
            "content_filters": [
                {"action": "redact", "pattern": "secret-\\\\d+", "label": "SECRET"}
            ]
        }
        """.utf8)

        let guardrail = try JSONDecoder().decode(Guardrail.self, from: json)
        #expect(guardrail.contentFilterBuiltins?.count == 1)
        #expect(guardrail.contentFilterBuiltins?.first?.slug == .email)
        #expect(guardrail.contentFilters?.first?.pattern == "secret-\\d+")
    }
    ...
    @Test func testEncodeCreateGuardrailRequest() throws {
        ...
        #expect(dict?["enforce_zdr_openai"] as? Bool == true)
        #expect(dict?["workspace_id"] as? String == "ws_abc")
    }
    ...
    @Test func testGuardrailIntervalValues() {
        #expect(GuardrailInterval.daily.rawValue == "daily")
        #expect(GuardrailInterval.weekly.rawValue == "weekly")
        #expect(GuardrailInterval.monthly.rawValue == "monthly")
    }
}
...
struct GuardrailsEndpointTests {
    ...
    @Test func testListGuardrailsEndpoint() {
        let endpoint = Endpoint.listGuardrails(offset: nil, limit: nil, workspaceId: nil)
    ...
    @Test func testListGuardrailsWithPagination() {
        let endpoint = Endpoint.listGuardrails(offset: "abc123", limit: 50, workspaceId: nil)
    ...
    @Test func testListGuardrailsWithWorkspaceFilter() {
        let endpoint = Endpoint.listGuardrails(offset: nil, limit: nil, workspaceId: "ws_123")
        #expect(endpoint.queryItems?.count == 1)
        #expect(endpoint.queryItems?.first?.name == "workspace_id")
        #expect(endpoint.queryItems?.first?.value == "ws_123")
    }
